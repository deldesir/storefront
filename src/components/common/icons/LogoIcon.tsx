...
export default function LogoIcon({ branding }: { branding?: LogoBranding }) {
  const storeName = branding?.name || process.env.NEXT_PUBLIC_SITE_NAME || "Store";
  const logoUrl = branding?.logoUrl || process.env.NEXT_PUBLIC_LOGO_URL || null;
  ...
  return (
    <div className="flex items-center gap-x-2">
      <div className="!min-h-10 !min-w-10 flex items-center justify-center rounded-lg border border-[#E0E0E0] bg-white dark:border-[#404040] dark:bg-black">
        {logoUrl ? (
          <Image
            src={logoUrl.startsWith("/") ? `${basePath}${logoUrl}` : logoUrl}
            alt={storeName}
            width={28}
            height={28}
